package nz.ac.auckland.se281.a3.bot;

import java.util.Random;

import nz.ac.auckland.se281.a3.Hand;
import nz.ac.auckland.se281.a3.Participant.Action;

public class HighRiskStrategy implements BotStrategy {
	
	/**
	 * Decides whether the bot hits or holds based on the score of its
	 * hand currently. This method will return HOLD if the score of the 
	 * bot's hand is between 19 and 21, otherwise it will return HIT.
	 * 
	 * @param hand hand of the bot
	 * @return the action the bot plays
	 */
	public Action play(Hand hand) {
		int score = hand.getScore();
		
		// check if the bots hand has a non-bust score of 19 or more
		if (score >= 19) {
			return Action.HOLD;
		} else {
			return Action.HIT;
		}
	}
	
	/**
	 * Randomly generates a number between 50 and 100 (inclusive)
	 * which will determine the size of the bet made.
	 * 
	 * @return the bet the bot makes. This bet will be between
	 * 50 and 100 inclusive
	 */
	public int bet() {
		// generate a random number between 50 to 100 inclusive
		Random random = new Random();
		return random.nextInt(51) + 50;
	}

}
